...
def display_menu():
    ...
    while True:
        print("=" * 56)
        print(BANNER)
        # Fixed quoting in f-string to avoid syntax error
...
def dial_number():
    ...
    number = input("Victim number: ").replace("+", "").replace("-", "").replace(" ", "")  # removing pluses and spaces for VonageAPI
...
def new_contact():
    ...
    contact_name = input("\nContact name: ")
    contact_number = input("Contact number: ").replace("+", "").replace("-", "").replace(" ", "")
        # removing pluses and spaces for VonageAPI
